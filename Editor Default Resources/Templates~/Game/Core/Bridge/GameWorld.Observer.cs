...
namespace Game
{
    ...
    public static partial class GameWorld
    {
        ...
        /// 观察者对象类型及其绑定函数的存储容器
        /// </summary>
        private static IDictionary<Type, IDictionary<string, NovaEngine.Definition.Delegate.EmptyFunctionHandler>> _worldObserverTypes = null;
        /// <summary>
        /// 观察接口与通知类型的映射容器
        /// </summary>
        ...
        private static void InitWorldObserverContext()
        {
            ...
            _worldObserverTypes = new Dictionary<Type, IDictionary<string, NovaEngine.Definition.Delegate.EmptyFunctionHandler>>();
            _worldObserverNamesMapping = new Dictionary<string, NovaEngine.Definition.Delegate.EmptyFunctionHandler>();

            //Array enumArray = Enum.GetValues(typeof(NovaEngine.Application.ProtocolType));
        ...
        private static void UnregisterAllWorldObservers()
        {
            ...
            IList<Type> observerTypes = NovaEngine.Utility.Collection.ToList(_worldObserverTypes.Keys);
        ...
        internal static void RegisterWorldObserver(Type observerType)
        {
            ...
            {
                Debugger.Error("The world observer class type '{%t}' must be inherited from 'IWorldObserver', registered it failed.", observerType);
                return;
            }
            ...
            IWorldObserver observer = Activator.CreateInstance(observerType) as IWorldObserver;
        ...
        /// </summary>
        /// <param name="observerType">观察对象类型</param>
        internal static void UnregisterWorldObserver(Type observerType)
        {
            Debugger.Assert(observerType.IsClass, "The world observer must be class type.");
        ...
        /// </summary>
        /// <param name="observerType">观察对象类型</param>
        private static void RegisterWorldObserverOnStaticClass(Type observerType)
        {
            ...
            foreach (KeyValuePair<string, NovaEngine.Definition.Delegate.EmptyFunctionHandler> pair in _worldObserverNamesMapping)
            {
                ...
                if (null == methodInfo)
                {
                    Debugger.Error("Could not found observer method '{%s}' from static class '{%t}', registered it failed.", pair.Key, observerType);
                    return;
                }

                NovaEngine.Definition.Delegate.EmptyFunctionHandler handler = (NovaEngine.Definition.Delegate.EmptyFunctionHandler) Delegate.CreateDelegate(typeof(NovaEngine.Definition.Delegate.EmptyFunctionHandler), null, methodInfo);
        ...
        /// </summary>
        /// <param name="observerType">观察对象类型</param>
        private static void UnregisterWorldObserverOnStaticClass(Type observerType)
        {
