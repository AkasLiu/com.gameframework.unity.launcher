...
namespace Game
{
    ...
    public static partial class GameWorld
    {
        ...
        /// 当前是否为教程模式
        /// </summary>
        static bool isTutorialMode = false;
        ...
        public static void Run()
        {
            ...
            if (NovaEngine.Configuration.TutorialMode)
            {
                // 教程开启
                isTutorialMode = true;
            }
        ...
        /// 游戏层的启动运行函数
        /// </summary>
        static async void RunGame()
        {
            // 加载应用上下文配置
            await GameLoader.Load(GameWorldCommandType.Context);

            // 装载热加载模块
            GameEngine.GameApi.AutoRegisterAllHotModulesOfContextConfigure();

            // 加载业务对象类
            await GameLoader.Load(GameWorldCommandType.Assembly);

            // 初始化上下文信息
            InitWorldObserverContext();

            IReadOnlyList<GameEngine.Loader.Symbolling.SymClass> list = GameEngine.Loader.CodeLoader.FindAllSymClassesByFeatureType(typeof(WorldObserverAutoBoundAttribute));
        ...
        /// </summary>
        /// <param name="commandType">类型标识</param>
        static async void ReloadGame(GameEngine.EngineCommandType commandType)
        {
            switch (commandType)
            {
                ...
                case GameEngine.EngineCommandType.Configure:
                    // 重新导入配置
                    await GameLoader.Reload(GameWorldCommandType.Configure);
                    break;
                default:
        ...
        const string TUTORIAL_MODULE_EXTERNAL_GATEWAY_NAME = "GameFramework.Sample.GameWorld";
        ...
        {
            CallRemoteModule(GameEngine.GameMacros.GAME_REMOTE_PROCESS_CALL_RUN_SERVICE_NAME);
        }
        ...
        {
            Type type = NovaEngine.Utility.Assembly.GetType(TUTORIAL_MODULE_EXTERNAL_GATEWAY_NAME);
            if (type == null)
            {
